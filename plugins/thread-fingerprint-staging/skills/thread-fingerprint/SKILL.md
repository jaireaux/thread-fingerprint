...
When the plugin is recognized but `identify_thread` is not callable, use error `TFP-HOST-TOOL-UNAVAILABLE`. Substitute one fresh NMCP reference everywhere `<legacy-reference>` appears and return this workflow:
